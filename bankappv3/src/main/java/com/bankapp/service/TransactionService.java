...
public class TransactionService {
	...
	public TransactionResponse withdraw(WithdrawRequest request) {
		...
		boolean requiresApproval = !isManager() && request.getAmount().compareTo(MANAGER_APPROVAL_LIMIT) > 0;
	...
	public TransferResponse transfer(TransferRequest request) {
		...
		if (requiresApproval) {
			...
			debitTx = Transaction.createPendingTransferDebit(sender, request.getAmount(), performedBy,
					receiver.getId());

			creditTx = Transaction.createPendingTransferCredit(receiver, request.getAmount(), performedBy,
					sender.getId());
			...
			debitTx = Transaction.createApprovedTransferDebit(sender, request.getAmount(), performedBy,
					receiver.getId());

			creditTx = Transaction.createApprovedTransferCredit(receiver, request.getAmount(), performedBy,
					sender.getId());
		}
		...
		return new TransferResponse(sender.getId(), sender.getName(), receiver.getId(), receiver.getName(),
				request.getAmount(), performedBy, debitTx.getCreatedAt());
	}
